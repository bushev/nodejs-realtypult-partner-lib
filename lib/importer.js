...
class Importer {
    ...
    constructor(options) {
         ...
         * Import report data
         *
         * @type {{location, statistics: {total: number, success: number, errors: number}}}
         */
        this.report = {
     ...
     * @param callback
     */
    removeOldReportFileIfExists(callback) {

        if (fileExists(this.reportFileLocation)) {

            fs.unlink(this.reportFileLocation, callback);
    ...
    parse(callback) {
        ...
        this.reader.on(`updateElement: ${this.itemTag}`, item => {
            ...
            this.onItem(item, (error, data) => {

                if (error) {

                    if (typeof error !== 'object') return callback(new Error(`Importer::onItem: error must be an object`));
                    if (!error.error) return callback(new Error(`Importer::onItem: 'error' must be provided`));

                    this.report.statistics.errors++;

                    this.xmlReport.push({object: [{_attr: {id: item.id}}, {error: error.error}]});

                } else {

                    if (!data.url) return callback(new Error(`Importer::onItem: 'url' must be provided`));
    ...
    storeReportFile(callback) {
        ...
        this.removeOldReportFileIfExists(err => {
